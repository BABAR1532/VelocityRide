...
async function connectMQ(retries = 0) {
  try {
    ...
    console.log('[Carpool Service] RabbitMQ connected');
    conn.on('close', () => { channel = null; setTimeout(() => connectMQ(), 3000); });
  } catch {
    if (retries < 10) { await new Promise(r => setTimeout(r, 3000)); return connectMQ(retries + 1); }
  }
}
async function publish(routingKey, payload) {
  ...
  console.log(`[Carpool Service] Published ${routingKey}`);
}
